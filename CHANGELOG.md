...
- Repository cleanup — consolidated outstanding local work onto one branch (`cprieger/overdue-cleanup`) and fixed the ignore rules that were blocking it:
  ...
  - Branch reconciliation: of ten local branches, nine held zero commits not already in `origin/main`. Only `cprieger/apache-watchface-v6` carried unique work (the V6 pass), so this branch was cut from it rather than cherry-picking — its commit is inherited intact. The remaining branches are redundant and safe to delete.
