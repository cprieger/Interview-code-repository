...
module Fonts {
    ...
    function timeFont() as Graphics.FontType {
        if (_time == null) {
            _time = load(FACE_NUMERIC, 52, Graphics.FONT_NUMBER_MEDIUM);
        }
    ...
    function secondsFont() as Graphics.FontType {
        if (_seconds == null) {
            _seconds = load(FACE_NUMERIC, 34, Graphics.FONT_NUMBER_MILD);
        }
    ...
    function dateFont() as Graphics.FontType {
        if (_date == null) {
            _date = load(FACE_STENCIL, 24, Graphics.FONT_MEDIUM);
        }
    ...
    function metricsFont() as Graphics.FontType {
        if (_metrics == null) {
            _metrics = load(FACE_NUMERIC, 20, Graphics.FONT_SMALL);
        }
    ...
    function headerFont() as Graphics.FontType {
        if (_header == null) {
            _header = load(FACE_STENCIL, 18, Graphics.FONT_SMALL);
        }
    ...
    function footerFont() as Graphics.FontType {
        if (_footer == null) {
            _footer = load(FACE_NUMERIC, 16, Graphics.FONT_XTINY);
        }
    ...
    function labelsFont() as Graphics.FontType {
        if (_labels == null) {
            _labels = load(FACE_STENCIL, 10, Graphics.FONT_XTINY);
        }
